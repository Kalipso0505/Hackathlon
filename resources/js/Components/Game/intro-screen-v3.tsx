/**
 * Intro Screen V3 - Vintage Case File with Crime Scene Photos
 * Authentisches FBI-Dokument im Stil der 60er Jahre
 * Split-Layout: Case File links, Polaroid-Fotos rechts
 */
...
// Evidence labels for the photos
const EVIDENCE_LABELS = [
    'CRIME SCENE - OVERVIEW',
    'EVIDENCE #1 - PRIMARY',
    'EVIDENCE #2 - SECONDARY',
];
...
export function IntroScreenV3({
    ...
    const hasImages = caseInfo.crimeSceneImages && caseInfo.crimeSceneImages.length > 0;
    ...
    return (
        <div className="h-screen bg-zinc-900 flex items-center justify-center p-4 overflow-hidden">
            ...
            {/* Main Container - Split Layout when images available */}
            <div className={cn(
                "relative flex gap-6 h-[calc(100vh-2rem)] max-h-[700px]",
                hasImages ? "w-full max-w-6xl" : "w-full max-w-2xl"
            )}>
                
                {/* LEFT: Document Container */}
                <div className={cn(
                    "relative h-full",
                    hasImages ? "w-[55%]" : "w-full"
                )}>
                ...
                {/* RIGHT: Crime Scene Photos - Polaroid Style */}
                {hasImages && (
                    <div className="w-[45%] h-full flex flex-col gap-4">
                        ...
                        <div className="flex items-center gap-2 text-zinc-400 font-mono text-xs uppercase tracking-widest">
                            ...
                            <span>Crime Scene Photography</span>
                        </div>

                        {/* Photos Grid */}
                        <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2">
                            {caseInfo.crimeSceneImages?.map((image, index) => (
                                <div
                                    key={index}
                                ...
                                >
                                    {/* Polaroid Frame */}
                                    <div 
                                        className="relative p-3 pb-12 rounded-sm transition-transform duration-300 group-hover:scale-[1.02] group-hover:-rotate-1"
                                        style={{
                                    ...
                                    >
                                        ...
                                        <div className="relative aspect-[4/3] overflow-hidden bg-zinc-800">
                                            <img
                                                src={image}
                                                alt={EVIDENCE_LABELS[index] || `Evidence ${index + 1}`}
                                            ...
                                            />
                                            {/* Evidence marker */}
                                            <div className="absolute top-2 left-2 w-6 h-6 bg-yellow-400 rounded-full flex items-center justify-center text-zinc-900 font-bold text-sm shadow-lg">
                                                {index + 1}
                                            </div>
                                            {/* Hover overlay */}
                                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                                                <span className="text-white opacity-0 group-hover:opacity-100 transition-opacity text-sm font-mono uppercase tracking-wider">
                                                    Click to enlarge
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="absolute bottom-2 left-0 right-0 text-center">
                                            <span 
                                                className="text-[11px] text-zinc-600 uppercase tracking-wider"
                                                style={{ fontFamily: 'Courier New, monospace' }}
                                            >
                                                {EVIDENCE_LABELS[index] || `EVIDENCE #${index + 1}`}
                                            </span>
                                        </div>

                                        {/* Tape effect on corners */}
                                        <div className="absolute -top-1 left-4 w-8 h-4 bg-amber-100/60 rotate-[-5deg] opacity-70" />
                                        <div className="absolute -top-1 right-4 w-8 h-4 bg-amber-100/60 rotate-[5deg] opacity-70" />
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="text-center">
                            <span className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">
                                ★ Photographic Evidence - Do Not Reproduce ★
                            </span>
                        </div>
                    </div>
                )}
